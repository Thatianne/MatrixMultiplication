...
void printLog(const char *filePath, const char *algoritmo, int n, double cpu_time, double exec_time, double read_time, double read_cpu_time)
{
	FILE *log;
	log = fopen(filePath, "a");
	fprintf(log, "%s,%d,%f,%f,%f,%f\n", algoritmo, n, cpu_time, exec_time, read_time, read_cpu_time);
	fclose(log);
}
